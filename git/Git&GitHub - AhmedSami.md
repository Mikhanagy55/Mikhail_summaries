...
# History of Git  
![[Pasted image 20250402161612.png]]
المعلم **Linus Torvalds**  بعد ما عمل ==**Linux**== ك open source, ف عايز بقى يحط ال Linux Kernel ده في مكان ما بحيث يكون `shared` والناس كلها ت connect عليه و تعدل كمان وت contribute و يرفع تعديلات. 
ف لجأ لواحده من الVersion control systems اللي موجودين و اختار واحد من اللي كانو متاحين واللي بيقدم خدمة for free. وكان اسمه **==bit keepers==**
...
حاجه ليك انت شخصيا. local عندك على الجهاز بتاعك الشغل بتاعك. انت بتعمل الVersion control لنفسك والحاجات اللي انت بتعدلها. لا حد بيشوفه ولا هترفعه ف حته ![[Pasted image 20250402152422.png]]
...
#### **Central Version Control** (***CVC***) 
 ...
 و كلنا بن Accessعلى ال server ده عشان نعمل التعديلات اللي عايزينها والتعديلات بتاعتنا بتبقى `live`. ![[Pasted image 20250402152851.png]]
...
ف عدلت شويه حاجات ف اقدر أupload او أpush التعديلات اللي عملتها ==بس==, اتعدل حاجات فال Server اقدر اخد الحاجات اللي اتعدلت ==بس==.
...
# GIT Basics 
ايه اللي يميز *==Git==* عن غيره, ايه المختلف بينه وبين الباقي.
...
في نظام زي **Subversion**، مش هيخزن الملف كله تاني. هياخد بس **الفرق**، يعني السطر اللي زوّدته ده بس، ويحتفظ بيه. دي اسمها **incremental differences** أو الدلتا (==delta==).
...
that what is called **==incremental or differential version control==**.
...
![[Pasted image 20250402220237.png]]
...
في git مش بيتقال عليه version control بنقول عليه **repository**  وعشان ندلعه ==git repo==.
...
![[Pasted image 20250402225503.png]]
...
**what is git object?**
==**هي الحاجات اللي git بيعملها tracking, في 4 حاجات بيعملها tracking.**==
1-**blob**
...
نعمل ده ازاي؟
 نروح للworking directory اللي فيه ال files وال folders اللي بتtrack it, الgit repo بقى اللي جواه كل الsnapshots وال objects, هيبقى جوه الWorking tree ده شخصياً. بيبقى `hidden folder`, اسمه ==git.== ده هيحول لي الproject folder كمان ويسميه `repo`.
...
![[Pasted image 20250403122821.png]]
...
![[Pasted image 20250403130628.png]]
ال**stdin--** عشان ياخد input من الpipeing.
الcommand اللي بيعمل نفس الكلام ده في Linux اسمه`shasum`.![[Pasted image 20250403130935.png]]طب ليه الاختلاف مع ان الاتنين بيستخدموا SHA1؟؟
...
ف هو بيشوفها كده ![[Pasted image 20250403132411.png]] الtype بعدين ال sizeاللي هو `11` مش 10 عشان في line break فالاخر مش ظاهر بس الterminal بتحطه. ثم null  character بعدين يحط الtext. 
اي تغير ولو ايه هيفرق لأن الAlgorithm شغال اصلا على ال ASCII وال binary format.

عشان Linux يشوف ال`0\` انه escape character مش مجرد text عادي بنضيف option `-e` على الecho command.![[Pasted image 20250403133207.png]]
...
![[Pasted image 20250403143540.png]]
...
طب ايه الCommit دي؟
معناها هو تrecord ألتعديل بتاع الobject فالrepo. اللي هو بعتمد التعديل ده فالversion control repo بتاعي. ![[Pasted image 20250403150600.png]]
...
![[Pasted image 20250404191354.png]]
...
![[Pasted image 20250403171557.png]]
>التسلسل الطبيعي
...
![[Pasted image 20250403174710.png]]
...
![[Pasted image 20250404191310.png]]
...
![[Pasted image 20250404185128.png]]
...
note 
حتى الحاجات دي ممكن اعدلها, ال`global--` معناها هي global بالنسبه للuser, يعني كل الprojects بتاعتك انت او اي folder تاني انت هتعمله initialization git repo على نفس الجهاز هياخد نفس الname والemail ده. وده بيعدل في file اسمه ==**gitconfig./~**==.
لكن لو عايز اعمله global بجد اللي هو على الSystem كله لكل الusers  فبنستخدم option تاني اللي 
هو `system--` وده بيعدل في file اسمه ==**etc/.gitconfig/**==.
وممكن اعمله على الlevel بتاع الproject ده بس من غير `global--` ولا `system--` وده بيعدل في file اسمه 
==**/.git/config**== 
![[Pasted image 20250404194808.png]]

لو عايز اشوف الConfiguratio هستعمل ![[Pasted image 20250404195540.png]]
كده انا جاهز اني استعمل Git.  فهروح ل الproject folder و أinitialize الgit repo,![[Pasted image 20250404202102.png]]خلاص عملت ألproject Folder وعملت file جواه, اشتغلت فيه يعني. عايز بقى أinitialize git جواه و احول الproject Folder ده الى Repository ,ف هعمل ده عن طريق اني اقوله `git init` لاحظ انك لازم تexecute ألcommand ده من جوه الproject folder. اول ما تexecute الCommand ده, بيتعمل folder مخفي إسمه `git.`، وده اللي بيخزن كل الـ versions والتاريخ بتاع الproject.![[Pasted image 20250404202848.png]]وحتلاقيه عمل ال folder اللي اسمه`git.` 
![[Pasted image 20250404203233.png]]
كده تَحول الproject بتاعك الى repo.
![[Pasted image 20250404203913.png]]
...
![[Pasted image 20250404205503.png]]
No commits yet:
...
احنا عندنا three-trees ال==working tree== وال==staging tree== اللي هو الindex وال==repo tree== اللي هو فيه الversions والcommits و الobjects وكل ده. عشان ال**three-tree architecture**.

طب لو انا عايز اشوف اللي في ده وفي ده, مثلا ابسط حاجه هاشوف اللي فالworking tree بالcommand `ls` ![[Pasted image 20250404232116.png]]
...
![[Pasted image 20250404232413.png]]
...
ممكن استخدم find command عشان اشوف حاجه معينه بدور عليها مثلا بدور على الobjects اللي من نوع file ![[Pasted image 20250404234348.png]]
يبقى كده عندي file واحد فالworking tree والrepo والindex فاضيين.![[Pasted image 20250404234603.png]]
...
الطبيعي اني هكتب `git add` وبعدها اسم الfile. ![[Pasted image 20250405000851.png]]
يعني لو 100 file فالproject هقعد اضيفهم file file ?!!
اقدر استعمل الregular expressions وال glob patterns بتاع الfile names ![[Pasted image 20250405001108.png]]
...
![[Pasted image 20250405001919.png]]
...
هانشوفه بقى فالindex لما نعمل `git ls-files`![[Pasted image 20250405002532.png]]
...
![[Pasted image 20250405002833.png]]
...
نشوفه في الRepo tree عن طريق الfind ![[Pasted image 20250405003635.png]]
اها اختلف اهو, بقى في عندي object مش object بس ده في folder اسمه `b7` كمان. ![[Pasted image 20250405012347.png]]لو ركزت شويه هتلاقي ان `b7` هما اول 2characters من ال`SHA1` بتاع الfile, وهتلاقي ان اسم الFile اللي جوه  `b7` هو باقي ال`SHA1` ![[Pasted image 20250405012718.png]]دي الطريقه الليgit بيsave بيها ال objects بيستعمل  `Folder structure`, مبيرميش كل objects كده على الroot بتاع الobjects directory,  تنظيم مش اكتر, وبيستعمل ال `SHA1` عشان يدي اسم للfolder والfile.
...
في command اسمه `git cat-file` نفس الcat العادي بس ده بيخليني اقرأ أل content بتاع الcompressed files بتاعة git وهستخدم option`-t` بعد كده ال`SHA1` بتاع الobject.![[Pasted image 20250405014651.png]]هو بيقرأ الcontent بس مش بيقرأ كله ال`t-`  بتخليه يقرأ الtype. عشان اشوف الsize هستخدم option`-s`.
لو تفتكر الshasum كان فيه size, type, and content .
وعشان الcontent هستخدم option `-p`
![[Pasted image 20250405015315.png]]

ده مش snapshot ده مجرد what is called `base blob` المعلم git بيستخدمه عشان يtrack الاختلافات.
كل ده جزء من ال indexing.
![[Pasted image 20250405015609.png]]
...
![[Pasted image 20250405021011.png]]
...
![[Pasted image 20250405021410.png]]
...
![[Pasted image 20250405134335.png]]
...
![[Pasted image 20250405150247.png]]
...
![[Pasted image 20250405152424.png]]
...
اذاً الcommit هي الwrapper اللي بيقولي ان الblobs او الtrees دول اتعدلت كده مع بعض في مره واحده .![[ Pasted image 20250405153305.png]]و بما ان هو object ف بيبقى ليه `sha1`.
...
![[Pasted image 20250405181152.png]]
...
>**note** 
مش كل مره هحتاج أrefer لأي object  هاقعد اعمل copy paste ل خرطوشه الcharacters دي, اقدر اكتبله اول كام character  وهو هيفهم, غالبا اقل عدد من الcharacters اقدر استخدمه اختزالاً ل ال`SHA1` هو 4characters. ممكن بالصدفه يتشابه 2objects فال`SHA1` ف ممكن نخليها 5 ونجرب يعني.  حتى git بيختصرها ب 7. ![[Pasted image 20250405182214.png]]

عرفت انه commit خلاص نشوف الcontent بتاعه بقى ممكن نستخدم `cat-file -p` ![[Pasted image 20250405182511.png]]هتلاقيه يعتبر نقس اللي ظهر فالstdout بتاع ال`git log`. و بيعرض الtimestamp بعدد الثواني اللي عدت.
...
![[Pasted image 20250405190623.png]]
 ...
 >**note** 
 الcommit object هو ده اللي بعد كده هنبدأ نشير ليه,  وكل Commit هعملها هنتكلم عن الcommit object بتاعها, ف هو ده الرأس والكبير بتاعهم مش هنقعد نقول trees و blobs ==**واللي مالوش كبير يشتريله كبير**==.
 
____________________________________________________________

# Basic Git operations(add, commit, log, diff, and show)

هجرب بقى اعدل فالfile واكتب line جديد. بعد ما تعمل save(==ctrl + s==) لو بتستخدم IDE زي **VS** هتلاقي اسم الfile اتغير وبقى مكتوب `M` اللي هو`modified` بس بالاحمر.
نشوف `git status` رأيه ايه بعد التغيير![[Pasted image 20250407155249.png]]قال لي الfile modified وبيديني اقتراحات بما انه اتعدل فبيقولي اعمله staging وبعدين Commit. وبيقولك برضو لو انت غلط و عملت staging وعايز تعمل unstaging  اعمل `git restore`.
وممكن استخدم `s option-` مع `git status` عشان يلخص الكلام ده, مثلا عندي 50 file هيبقى موال بقى. ![[Pasted image 20250407155809.png]]حتة انه modified بالاحمر دي يعني الfile اللي عندي فالWorking tree على الOS لا بيساوي اللي فالindex ولا بيساوي اللي موجود فالgit repo.
الline أللي زودته ده مش موجود فالstaging area ولا موجود فالgit repo.

الnext step الطبيعيه هي اني هعمله `git add` فيبقى modified بالاخضر ![[Pasted image 20250407160337.png]]وده معناه الfile اللي فالworking tree على الOS هو نفسه اللي فالStaging area `index` لكن مش نفس اللي فالgit repo. 
...
![[Pasted image 20250407160848.png]]
...
![[Pasted image 20250407161145.png]]

نبص على الobjects اللي موجوده عندي كلها بعد ماعملت commit كمان ![[Pasted image 20250407161319.png]]كله سايح ف بعضه, تخيل بقى عندك 5او6 commits, ف كان لازم يبقى فيه حاجه توريني انهي objects من دول Related مع بعض واتعملوا Commit واحده. 

نشوف المحتوى بتاع الcommit دي ونشوف الblobs وكل اللي موجود ![[Pasted image 20250407161808.png]]ايه ال
parent ده. ده جديد مكنش موجود فال commit اللي قبل  دي
![[Pasted image 20250407162128.png]]
...
![[Pasted image 20250407165316.png]]
...
اول commit خالص ملهاش parent واسمها ال`root commit`.![[Pasted image 20250407170003.png]] الشكل ال linear ده, that's what is called `branch`.
...
________________________________________________________________________________
![[Pasted image 20250407215311.png]]
...
بيديني list بكل الفروق في كل الFiles و الfolders أللي فالproject. ![[Pasted image 20250407224024.png]] هنا مثلا بيقولي في line اتضاق ملونه بالاخضر وعامل جمبه `+`. و بيعمل `-` للأسطر اللي اتمسحت.
 
هعمل الstep اللي بعد كده `git add` كده اللي عندي فالWorking tree هو هو اللي فالindex.
اجرب `git diff` تاني,![[Pasted image 20250408012350.png]]مطلعش حاجه!!
...
و  `staged--` معناها git command بين الrepo والindex. مالوش علاقه بالworking tree.![[Pasted image 20250408013112.png]]وهو كان اصلا نفس الفرق بين الindex والworking tree قبل ما اعمل `git add`.
...
![[Pasted image 20250408014257.png]]
هكتب بس git commit هيفتحلي على طول الeditor.
![[Pasted image 20250408014344.png]]
بعد الsubject لو عايز ازد ممكن انزل line اضيف description بس حاول مايزيدش عن 70 characters.
![[Pasted image 20250408014623.png]] 
و أsave عادي زي اي editor.
وهاشوف الsubject والdescription عادي لما اعمل `git log`![[Pasted image 20250408014807.png]]
ترويشه على `git log`
1- ==**--oneline**==
![[Pasted image 20250408014910.png]]
...
مثلاُ عايز اقوله اعملي filtering لل commits اللي فيها الblobs بتاعة  `file.txt`![[Pasted image 20250411210401.png]]بحيث لما يكون عندي عدد commits كتير جدا اقدر أfilter اللي انا عايزه بس, مثلا انا شغال في الحاجات اللي ليها علاقه بالdatabase او json files بس. ماليش دعوه بالباقي مثلا 

2- ==**-n**==
بحيث اقوله عايز اخر كام `n` دول بس.![[Pasted image 20250411210734.png]]   ولو عايز الcommits بتاعة حد معين 
...
![[Pasted image 20250411211707.png]]
![[Pasted image 20250411212237.png]]
...
![[Pasted image 20250411212345.png]]
________________________________________________________________________________
ممكن اعمل graph لما استخدم `graph--` ![[Pasted image 20250411212718.png]]
________________________________________________________________________________
...
![[Pasted image 20250412002627.png]]
...
`git diff SHA1..SHA1`
![[Pasted image 20250412003653.png]]
هنا بيقولك ان الفرق بينهم ان في 2files اتضافوا.
![[Pasted image 20250412003823.png]]
...
مش معنى اني مسحته خلاص لا, ان اهم مزايا git أن مفيش بتتمسح لو مسحت حاجه مش هيتشال وخلاص ده هيتكتب انك مسحت الline ده.
 لو جيت مسحت File سواء بgit command او من الoperating system لازم اعمله staging و commit.
  ![[Pasted image 20250412005538.png]]
## moving or rename files 
احياناُ git بيهيس لما اعمل Rename او move من الOS, فالاحسن ان انا اعمل كده ب`git mv` ![[Pasted image 20250412014334.png]]
## viewing the commit history 
![[Pasted image 20250412014440.png]]
...
لو عايز أ delete الrepo بسهوله خالص,هعمل delete الfolder بتاع الrepo.![[Pasted image 20250412021856.png]] كده خلاص مبقاش في repo, نبتدي على نضافه بقى.
...
![[Pasted image 20250412022344.png]]
...
لو جربت اعمل `git status`![[Pasted image 20250412075829.png]]
...
________________________________________________________________________________
![[Pasted image 20250412081125.png]]
...
نبدأ بالHEAD 
![[Pasted image 20250412084451.png]]
ده الوضع دلوقتي 
...
الarchitecture دلوقتي ![[Pasted image 20250412085833.png]]
...
او او متأكد يبقى مره واحده وخلاص اجيبه من الrepo على طول على الworking tree.![[Pasted image 20250412090951.png]]
...
________________________________________________________________________________
![[Pasted image 20250412092628.png]]
الHEAd ده عباره عن file جواه Pointer بيشاور على `SHA1`. لو روحت ل `git.` وعملت ls هلاقي في file اسمه `HEAD`.![[Pasted image 20250412092822.png]]هلاقي جواه زي لينك كده 
![[Pasted image 20250412092938.png]]
...
ممكن مثلا لو عايز اقارن بين الHEAD واي commit تانيه بس انا مش عارف الSHA1 بتاع الHEAD.![[Pasted image 20250412094728.png]]المفروض لو رجعت للcommit دي ال2lines الفرق دول هيتشالوا.
![[Pasted image 20250412095739.png]]
...
و reflog هي short for `refrences log`, فهي بتوريني الlog بتاع الrefrences, الRefrences هنا زي ال`SHA1` كده.![[Pasted image 20250412102139.png]]كل حاجه معروفه و مرصوده 
...
الobject 3c183d4 اختفى من git log, ممكن اتأكد انه موجود عن طريق `git show`![[Pasted image 20250412133745.png]]
...
![[Pasted image 20250412134006.png]]

واقدر اشوف النتيجه من `git log` وايه اللي حصل من `git reflog`![[Pasted image 20250412134247.png]]
![[Pasted image 20250413101226.png]]
________________________________________________________________________________
![[Pasted image 20250413102320.png]]

`git revert`
Given one or more existing commits, revert the changes that the related patches introduce, and record some new commits that record them. This requires your working tree to be clean (no modifications from the HEAD commit).

Note: `git revert` is used to ==_record_== some new commits to reverse the effect of some earlier commits (often only a faulty one).it help undo commits that have already been pushed and shared with the team, we cannot use the git reset command. Instead, we have to use `git revert`. git revert will create a new commit that will undo all of the work that was done in the commit you want to revert.
...
![[Pasted image 20250413104545.png]]
...
![[Pasted image 20250413104647.png]]
_________________________________________________________________________________
This makes the following two commands equivalent:
![[Pasted image 20250413104713.png]]

________________________________________________________________________________
This seems a bit clearer. It’s also common to add a last command, like this:![[Pasted image 20250413104823.png]]This way, you can see the last commit easily:![[Pasted image 20250413104856.png]]
_________________________________________________________________________________
...
![[Pasted image 20250413105029.png]]
**Summary**
...
![[Pasted image 20250414013007.png]]
...
![[Pasted image 20250414015042.png]]
...
يعني لما اعمل Switch ,تلقائي الHEAD هيشاور على الcommit بتاعة الmaster.![[Pasted image 20250426125835.png]]و ده هيخلي اللي فالworking tree يبقى هو هو أللي فالcommit بتاعة الmaster. 
فأصبح ان الHEAD بيشاور على commit قبل اللي كانت موجوده 
![[Pasted image 20250426130414.png]]
...
في option اسمه `merged--` مع `git branch` بيوريني الbranches اللي اتعملها merge في الmaster.![[Pasted image 20250426172624.png]]
خلاص وظيفه الtesting branch ممكن امسحه عادي كده. ==d = delete-==![[Pasted image 20250426172709.png]]
...
الmerge هنا اسمه `three-way merge` وده بيعتمد على التلاته دول![[Pasted image 20250426183854.png]] عشان يعمل الmerge, بيعمل COMMIT جديد revision رابع ولا واحد منهم.

![[Pasted image 20250426184439.png]] 
هعمل بقى merge للtesting into master, فلما اعمل ده هيفتحلي text editor يقولي اكتب message عشان ال`merge commit` بقى.

ف ده اللي هيحصل
![[Pasted image 20250426184833.png]]
![[Pasted image 20250426184857.png]]

مكنش فيه خط linear بين الbranches فGit ااضطر يعمل revision جديد عشان يقدر يعملهم merge وادالها commit message. 

![[Pasted image 20250426185128.png]]
كده خلاص Testing اتعلمه merge في الmaster, ف مش محتاجه ممكن امسحه عادي.
________________________
لو شوفت الHEAD  ![[Pasted image 20250426185321.png]] هتلاقيه بيقولك انه merge بين الcommits. و هما دول أللي كان بيشاور عليهم ال2branches.
____________
...
ف هعدل فالfile واعمل اللي انا عايزه.و بعد ما اعدل الملف وامسح العلامات، هحفظ التغييرات.ف هاعمل ==_git add_== ثم ==__git commit__== ف يتعمل الmerge commit.
لما بيحصل الconflict ف git بيعدل الfile عن طريق انه ويحط علامات (`<<<<<<<`, `========`, `>>>>>>>`) عشان يوضح التغييرات من الفرعين:
- التغييرات من master هتبقى تحت ==_<<<<<<< HEAD_==.
- التغييرات من testing هتبقى تحت ==_>>>>>>> testing_==.
- بينهم فاصل (`=======`).
- ![[Pasted image 20250426234859.png]]
 **Merge Conflicts**
...
![[Pasted image 20250426235950.png]]
***___________________________________________________________________________________________***
...
![[Pasted image 20250427003106.png]]
___________________

اقدر أconnect على اكتر من remote.  ![[Pasted image 20250427003259.png]]مثلا في اكتر من شخص على مستوى العالم شغلاين على نفس الrepo وكل واحد عنده النسخه بتاعته بيعدل فيها فأقدر اخد دايما الupdates او أpush التعديلات بتاعتي.
...
عشان اعلم cloning هستخدم `git clone` واديله الlocation بتاع الrepo دي. هو تلقائي بيدي الlocal repo  نفس الاسم بتاع الremote لو لسبب ما كنت عايز اغير الاسم بتاع الlocal repo عندي هاكتب الاسم اللي انا عايزه بعد الlocation.![[Pasted image 20250427004919.png]]
عشان اشوف كل الreomte repos اللي عندي هستخدم `git remote`. ![[Pasted image 20250427005439.png]]ال==_origin_== ده اسمه الshort name. لما اشوف origin دي كده ده معناه ان الrepo أللي شغال عليه ده متاخد من remote repo. 
الorigin ده  اسمه الshort name او ال`alias` بتاع الremote repo. لو عايز اعرف details عن الremote repo ده.  
هستخدم `git remote -v` ![[Pasted image 20250427010213.png]]هلاقي فيه اتنين origin والorigin دي زي alias كده عن الURL بدل ما كل مره  هكتب الURL كله.
...
ده الremote![[Pasted image 20250427040606.png]]
وده الlocal![[Pasted image 20250427040609.png]]

هاجي اشوف ده فالlocal مثلا اجرب `git status` مش هيحس بيه ![[Pasted image 20250427033103.png]]وده مش صح. ازاي انا up to date مع الremote.
لكن ده طبيعي لأنك لسه مجبتش الupdates اللي حصلت فالremote. هي دي اخر حاجه يعرفها الlocal repo انك up to date.

ف by the way, عشان اعرف الbranches اللي جايه من الremote. هنستخدم `git branch -r`.![[Pasted image 20250427033502.png]]
...
![[Pasted image 20250427034650.png]]
بعد `git fetch` هو عرف ان  في commit جديده اتعملت فالremote بس ليه مطبهاش فالWorking tree عندي فالlocal repo.
حتى لما عملت cat-file عندي فالlocal Repo ل الrevision اللي اتضاف فالremote repo لاقيته موجود.![[Pasted image 20250427035757.png]]اللي بيحصل ان `git fetch` بتعمل download لكل الupdates والrevisions ألمختلفه عن الlocal لكن مبتعملهاش `merge` مع الlocal repo والworking tree عندي.
بحيث يبقى في Work flow حتى اعمل `git show` واشوف ايه الدنيا. لو تمام خلاص اعمل `git merge`. 

ف by the way لو جربت اعمل `git status` بعد ما عرف updates لما عملت `git fetch` هلاقيه جابلي الرساله التمام.![[Pasted image 20250427040353.png]]
لما اعمل `git merge` هيعمل update ل الworking tree.![[Pasted image 20250427041027.png]]
و git status خلاص مفيش حاجه انت up to date وتمام.![[Pasted image 20250427041327.png]]
طب لو جربت اعمل تعديل فالlocal. ألlocal repo عارف كل حاجه, هيقولك انت اللي متقدم Commit عن الremote.![[Pasted image 20250427041522.png]]
![[Pasted image 20250427041646.png]]
...
حتى اهو الremote معندوش غير 2commits ![[Pasted image 20250427042340.png]]
 ...
 **typical work flow.**
  ...
  ==اغلب الremote repos بتبقى لمجرد الdevelopment ف ملهاش working tree من الاساس.==
__________

لو جيت عملت branch في الLocal وعملتله switch  ثم عملتله commit.![[Pasted image 20250427045351.png]]هلاقي ان Git مش شايف ومش فاهم ايه علاقه branch feature بالremote. 
...
كده عندي branch كامل مش موجود فالRemote. ف هعمل بقى ==up stream== عن طريق
`<اسم البرانش> git push`. <اسم البرانش> اللي ه push عليه.![[Pasted image 20250427123638.png]]جابلي الerror ده `the current branch feature has no upstream branch.`.
...
==__--set-upstream__== = ==_-u_==.
so we can type `git push -u origin feature`
بقوله وانت بتpush اعمل branch اسمه feature فالorigin. اذا الcommand ده هيعمل feature branch وي synchronize الupdates بتاعته.![[Pasted image 20250427125047.png]]![[Pasted image 20250427125013.png]]
واتأكد من ده لما اعمل `git branch` فالremote.
![[Pasted image 20250427125202.png]]

كده تمام بالنسبه `git status`
![[Pasted image 20250427125425.png]]
 هتلاحظ انه المره دي اداني tracking information بين الlocal والremote عن الfeature branch.
  ...
  ___________
  عشان ن synchronize من الremote ل الlocal, عملنا `git fetch` ثم `git merge` الخطوتين دول اقدر اعملهم ب command واحد بس اللي هو `git pull` ![[Pasted image 20250427130543.png]]![[Pasted image 20250427130545.png]]
  ___________
  
![[Pasted image 20250427130832.png]] 
بيlist كل الbranches اللي عندي والlatest commit بتاعتهم و بيحددلي ب `*` ان ده الHEAD.

![[Pasted image 20250427130933.png]] 
...
عشان أclone repo من GitHub ![[Pasted image 20250427143550.png]]
لو نزلت الzip file هاتروح تعمله extract عادي لو فتحته من الterminal هتلاقي في hidden directory اسمه `github.` ![[Pasted image 20250427143849.png]]بس مفيش `git.` الrepo يعني.
...
![[Pasted image 20250427144515.png]]
ولما اخش الdirectry هلاقيه عرف انه repo ![[Pasted image 20250427144548.png]]
...
______
![[Pasted image 20250427201611.png]]
...
وهلاقي تأثير ده حصل هنا اهو![[Pasted image 20250427203505.png]]قالك عندك 2 commits وضاف الfile اهو, وال`SHA1` بتاع الlatest commit.
لو دوست على Commits هيظهرلي الcommits اللي عندي كده ![[Pasted image 20250427203803.png]] لو دوست على ال`SHA1` بتاع اي واحده هايقولي ايه اللي حصل فيها.![[Pasted image 20250427203906.png]]
...
_________
لو عايز أcreate Branch هاروح عند كلمه `main` و اكتب اسم الbranch ده.![[Pasted image 20250427211543.png]]و أclick على `create branch: feature`.
الbranch اللي اسمه بيبقى مكان الmain كده هو ده ال==_current branch_==.
...
عادي نعمل upstream لو اسامي الbranches مختلفه بس ده مش كويس. ف نسمع كلامه تاني ونغير اسم الmaster ل main![[Pasted image 20250427225144.png]]
خلاص كله تمام نعمل اخر حاجه بقى اللي هو نعمل push. عن طريق `git push -u origin main`

![[Pasted image 20250427225326.png]]
...
note 
![[Pasted image 20250428020226.png]]
...
![[Pasted image 20250428020438.png]]
...
![[Pasted image 20250428022154.png]]
...
هgenerate ألkey عن طريق الcommand ده![[Pasted image 20250428023833.png]]لازم تتأكد ان الايميل اللي هتكتبه هنا اللي هو على اساسه هيتعمل الkey, يكون هو نفس الemail اللي انت عملت بيه ال configuration ل الgit repo locally عندي.

![[Pasted image 20250428025816.png]]
...
![[Pasted image 20250428031416.png]]
![[Pasted image 20250428031444.png]]

بيقولي تقدر كمان تعمل evaluation تتأكد ان الkey اتعمل صح. ![[Pasted image 20250428133037.png]]الPID بيظهر عشان يأكد إن الـ Agent شغال, وتقدر تستخدم الرقم ده لو عايز تتحكم في العملية (مثلاً توقّفها أو تشوفها).
...
![[Pasted image 20250428133856.png]]
وهاخد الcontent كله copy وارجع ل GitHub و اخش على الssh key setting و أclick على ==__New SSH Key__==.
اديله اي اسم ,واحط الkey ![[Pasted image 20250428134116.png]]وأclick على ==__Add SSH key__==.

نعمل push بقى.
![[Pasted image 20250428134950.png]]
الpush المره دي نجح.

اتأكد برضو لما تلاقي الlocal والremote عندهم نفس الcommits بلا`SHA1` بتاعهم.![[Pasted image 20250428140237.png]]![[Pasted image 20250428140252.png]]
كده الpush successful.

![[Pasted image 20250428140612.png]]
وهنا بيقولك ان تم استعماله بالفعل.

اقدر اخش على الseeting وبتاعه الrepo واعملله delete.

![[Pasted image 20250428030442.png]]
...
**What is the SSH Agent?**
 ...
 If you have multiple keys or issues with your old key tied to an IP, configure ==_~/.ssh/config_== to ensure the correct key is used.
...
ف محتاج اني اعمل `fetch` عشان اقدر اشوف التعديلات. ![[Pasted image 20250428142718.png]]كده عملت fetch لو عملت git log ![[Pasted image 20250428143222.png]]
ممكن استخدم `all--` عشان يوريني كل الcommits الموجوده سواء remote او local.
![[Pasted image 20250428142836.png]]
هيقولي في واحده ذياده 

عشان الذياده دي اشوفها عندي فالlocal هعمل `git merge`
![[Pasted image 20250428143408.png]]

لو عملت git log هلاقي ال4commits الموجوده.![[ Pasted image 20250428143446.png]]
أقدر اpublish من على الVS code ![[Pasted image 20250428145326.png]]
...
او الطبيعي والمنطقي ==_اني اعمل cloning من الforked repo بتاعي_== ده.  ![[Pasted image 20250428152203.png]]![[Pasted image 20250428152232.png]]

ولو عملت `git remote -v`![[Pasted image 20250428152356.png]]هلاقي ان الremote بتاع الrepo ده هو من الaccount بتاعي. مش الoriginal اللي واخد منه الfork.

وممكن اضيف remote تاني اللي هو الشخص اللي واخد منه الfork .
![[Pasted image 20250428152744.png]]
...
عشان اعمل ده هاروح ل==pull request==![[Pasted image 20250428153905.png]] و أclick على ==__Create pull request__== 
ف هيشوف التعديلات اللي انا عدلتها ومش موجوده فالoriginal ![[Pasted image 20250428154009.png]]و أclick على ==__Create pull request__==

واحط الmessage وابدأ اديله تفاصيل عن اللي عملته, و أclick على ==__Create pull request__==.

عند الراجل هناك بقى هيفتح الpull request هيلاقي في request جايله منahmed sami ![[Pasted image 20250428154309.png]]
و يا اما يقبله او ي close انه ي ignore يعني.
ويحط comment 
![[Pasted image 20250428154601.png]]
